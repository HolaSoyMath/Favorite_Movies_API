...
import { checkIdMovieExistsOnTmdb, checkIdUserExists } from "../../../../utils/checkExists";
...
export class DeleteFavoriteMovieUseCase {
    async execute ({ userId, movieId }: AddFavoriteMovieDTO): Promise<Favorite[]> {
        checkIdUserExists(userId);
        isNumber({movieId});
        checkIdMovieExistsOnTmdb(movieId.toString());
        ...
        if(!isFavoriteMovieLinkedToUser){
            throw new AppError("Favorite movie not linked to selected user", 404)
        }
